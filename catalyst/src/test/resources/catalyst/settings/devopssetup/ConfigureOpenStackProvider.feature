...
Feature: As a super user I configure Open Stack provider, Edit and Delete it
  ...
  Scenario Outline: As a super user I create a Open Stack Provider
    ...
    And I click on browse open stack pem file and select "key_rldc_new_openstack.pem" file
    And I select "<Organization>" from the select box in new provider page
#    And I select "<Organization>" from the select box
    And I click on save button
    And I verify "<Open Stack Provider name>" in the provider table
    And I verify "<Open Stack Provider name>" with "<Organization>" in the provider table
    And I verify "<Open Stack Provider name>" with "<Provider Type>" in the provider table
    And I verify "<Provider Type>" in the provider table
    And I verify "<Organization>" in the provider table
    And I select the "<Open Stack Provider name>" and verify "Edit" Button is enabled
    And I select the "<Open Stack Provider name>" and verify "Delete" Button is enabled
...
#      |  RL_Open_Stack         |    OpenStack_Pro       |
  ...
  Scenario Outline: As a super user I delete the created a Open Stack Provider
    ...
    And I verify created "<Open Stack Provider Name>" is deleted
