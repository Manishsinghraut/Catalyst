...
public class ImportNodesFromChefViews {
    ...
    public static void clickOnSoftwareStack() {
        try {
            SeleniumUtil.waitForElementIsClickable("id", "grid0", 8, SeleniumUtilities.OBJWAITTIMEOUT);
            SeleniumUtil.click("id", "grid0", SeleniumUtilities.OBJWAITTIMEOUT);
        } catch (Exception ex) {
            BaseView.takeScreenshot("clickOnSoftwareStack");
            SeleniumUtil.Log.info("Error :" + ex.getMessage());
            fail(ex.getMessage());
        }
    }
    ...
    public static void verifyEnvDetails(String envDetails) {
        try {
            ...
            if (SeleniumUtil.waitForElementContainText("xpath", "//span[contains(text(),'" + envDetails + "')]", envDetails)) {
                logger.info(envDetails + " is present");
            } else {
                logger.info(envDetails + " is not present");
            }
        } catch (Exception ex) {
    ...
    public static void importPemFileInPopupWindow() {
        try{
            ...
            driver.findElement(By.id("template")).sendKeys(path);
        }
        catch(Exception ex){
