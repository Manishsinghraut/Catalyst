...
//            fail(msg);
    ...
    public void I_Login_to_catalyst_using_access_credentials(String par0) throws Throwable {
        try {
            boolean flag = false;
            String strUserName, strPassword;
...
//            strPassword = "superadmin@123";//CucumberContext.getPassword();
            ...
            strUserName = properties.getProperty("iwms.username", "not_filtered");

            strPassword = properties.getProperty("iwms.password", "not_filtered");
            ...
            BaseView.takeScreenshot("I_Login_using_access_credentials");
            String msg = String.format("Unable to login and see expected dashboard: %s", e.getMessage());
            logger.log(Level.SEVERE, msg, e);
            logger.info("Created new WebDriver instance as old one was failing to respond.");
        }
