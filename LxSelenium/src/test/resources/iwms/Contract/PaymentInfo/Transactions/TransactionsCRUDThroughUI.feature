...
Feature: To ensure that users can add, edit, delete a Transactions.
...
#      | Category         | Rent              |
  ...
  Scenario Outline: I cannot edit processed Transactions base amount
    Given I Login to IWMS using "<Login User>" access credentials
    Then I check previous user login "<Login full username>" and continue if user "<Login User>" same or logout and login if not
    When I see the Dashboard
    And I navigate to entity type "<Item>" named "<Entity Name>"
...
#    And I click row from grid "<Grid ID>" having unique value "<Effective Date>" under column header "Effective Date"
    ...
    And I set following fields value on editable page
      | name                                       | value |
#      | PaymentTransaction_InvoiceAmount           | 300   |
...
#    Then I verify "Unable to change Payment value(s) when marking the Payment as processed" error message appear in top left corner of page
  ...
  Scenario Outline: I can edit processed Transactions comment
    Given I Login to IWMS using "<Login User>" access credentials
    Then I check previous user login "<Login full username>" and continue if user "<Login User>" same or logout and login if not
    When I see the Dashboard
    And I navigate to entity type "<Item>" named "<Entity Name>"
  ...
  Scenario Outline: I cannot delete processed Transactions
    Given I Login to IWMS using "<Login User>" access credentials
    Then I check previous user login "<Login full username>" and continue if user "<Login User>" same or logout and login if not
    When I see the Dashboard
    And I navigate to entity type "<Item>" named "<Entity Name>"
  ...
  Scenario Outline: I can delete unprocessed Transactions
    Given I Login to IWMS using "<Login User>" access credentials
    Then I check previous user login "<Login full username>" and continue if user "<Login User>" same or logout and login if not
    When I see the Dashboard
    And I navigate to entity type "<Item>" named "<Entity Name>"
    ...
    Then I verify row from grid "<Grid ID>" having unique value "<Effective Date>" not present
