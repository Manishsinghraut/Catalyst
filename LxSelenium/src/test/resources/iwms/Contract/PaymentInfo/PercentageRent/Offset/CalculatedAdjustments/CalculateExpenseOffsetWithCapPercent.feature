@all
Feature: Calculated Expense Offset with cap percent (Ref : Jira Ticket No : Automation-766)

  Scenario Outline: Import the attached Calculated Expense Offset with Cap percent contract xml
    ...
    And I import following files:
      | CalculatedExpenseOffsetWithCapPercentage.xml |
  ...
  5. Navigate to Payment Info->Sales and ensure that sales data exist for 12 period for 2014
    ...
    And I set fields values on editable page
      ...
      | VariableRentOffset_CapPercent | 30         |
    ...
    Examples:
      ...
      | Contract | Payment Info     | Calculated Expense Offset with Cap Percentage |
  ...
  Scenario Outline: As a delete user I process payment aand verify them in transactions page
    ...
    And I wait for the Loading text to disappear
    And I select "March" month and "2015" year from dropdown in schedule details to view the details
    And I click on offset amount link "$900.00"
    And I see modal window

    And I verify a unique row in grid "^Grid_" having unique value "03/01/2015" under column header "Effective Date"
      | name         | value                |
      | Group        | Lease Expense        |
      | Type         | Building Maintenance |
      | Category     | Expense              |
