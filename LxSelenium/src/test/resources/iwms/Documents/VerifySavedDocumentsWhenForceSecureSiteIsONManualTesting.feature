@all
Feature: Verify saved documents when Force secure site is ON - Manual Testing (Ref : Jira Ticket No : AUTOMATION-1116)
  ...
  Scenario Outline: As a LxSelenium user I import files
    ...
    And I import following file name:
      | FolderTemplate_AUTOMATION-1116.xml                   |
      | Project_for_Force_secure_site_ON_AUTOMATION-1116.xml |
      | ForceSecureSiteIsONReport_AUTOMATION-1116.xml        |
    And I logout
  ...
  Scenario Outline: As DeleteAccess user, I verify saved documents when Force secure site is ON - Manual Testing
    Given I Login to IWMS using "<Login User>" access credentials
    Then I check previous user login "<Login full username>" and continue if user "<Login User>" same or logout and login if not
...
#      | name        | value                          |
#      | Report Name | Force secure site is ON Report |
#    And I switch to "report" child window
#    And I click on the "Save To Documents" link
#    And I switch to "Save Documents" third child window
#    And I set following fields value on editable page
#      | name        | value             |
#      | portfolioID | Lucernex-Selenium |
#    And I set following fields value on editable page
#      | name     | value                           |
#      | FolderID | SeleniumTestTemplateRootFolder1 |
#      | BaseName | <File Name>                     |
...
#    And I switch to main window
  ...
  Examples:
    ...
    | DeleteAccess | Delete Access Access | Lucernex-Selenium | ForceSeureSiteIsOn |

  Scenario Outline: As DeleteAccess user, I verify saved documents when Force secure site is ON - Manual Testing1
    Given I Login to IWMS using "<Login User>" access credentials
    Then I check previous user login "<Login full username>" and continue if user "<Login User>" same or logout and login if not
...
#    And I click on the "Documents" tab
    ...
    And I select the following
      | SeleniumTestTemplateRootFolder1 (1) |
    ...
    And I verify the following text in a page
      | <Entity Name>                       |
      | Project for Force secure site is ON |
...
#    | DeleteAccess | Delete Access Access | Lucernex-Selenium |
  Examples:
    | Login User   | Login full username  | Entity Name       |  Report Name                    | Grid        |
    | DeleteAccess | Delete Access Access | Lucernex-Selenium |  Force secure site is ON Report | pageContent |
