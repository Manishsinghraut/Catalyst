...
public class SeleniumUtilities {
    public static int OBJWAITTIMEOUT = 1;
    public static int WEBDRIVER_WAIT = 60;
    public static int WEBDRIVER_WAIT_SMALL = 30;
    public static int DEFAULT_TIMEOUT_IN_SECONDS = 4, DEFAULT_SLEEP_TIME_IN_SECONDS = 4, WAIT_TIME = 4;
...
//            action.click(ele).perform();
    ...
    public void dragAndDropDocuments(String fromElementLocator, String fromElement, String targetElementLocator, String targetElement) throws Exception {
        try {
            int count = 0;
            WebElement fromEle = getWebElementObject(fromElementLocator, fromElement);
            WebElement targetEle = getWebElementObject(targetElementLocator, targetElement);
            Actions builder = new Actions(driver);
            builder.clickAndHold(fromEle)
                    .moveToElement(targetEle)
                    .release(targetEle).click().release().build().perform();
            /*while (BaseView.isElementPresentNoWait(targetEle) && count <5) {
                count++;
                builder.release(targetEle).click(targetEle).build().perform();
            ...
            BaseView.takeScreenshot("Unable to drag and drop the element.png");
            BaseView.popDefaultTimeout();
            Log.error("Error Message :" + ex.getMessage());
        }
    }
        ...
        try {
            ...
            for (int i = 0; i < 10; i++) {
                ...
                try {
                    ...
                    wait.until(ExpectedConditions.visibilityOfElementLocated(By.cssSelector("div#folderMenu-body")));
    ...
    public void rightClickSelectMenuOption(String locator, String element, String menuLocator, String menuElement) throws Exception {
        try {
            ...
            for (int i = 0; i < 10; i++) {
                ...
                try {
                    WebDriverWait wait = new WebDriverWait(BrowserDriver.getCurrentDriver(), 5);
                    wait.until(ExpectedConditions.visibilityOfElementLocated(By.id("mainMenu-innerCt")));
    ...
    public void rightClickSelectMenuOption(WebElement ele, String strMovemenuOptionLink, String menuOptionLink) throws Exception {
        ...
        do {
            ...
            try {
                ...
                WebDriverWait wait = new WebDriverWait(BrowserDriver.getCurrentDriver(), 5);
                wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath("//span[text()='" + menuOptionLink + "'][contains(@class,'x-menu-item-text')]")));
...
//    }
    ...
    public boolean verifyDataInTableRow(WebElement ele, String strLabelName, String strValue) throws Exception {
        ...
        for (WebElement trElement : tableRows) {
           /* if((replaceSpecialChar(trElement.getText()).trim().contains(strLabelName)) && replaceSpecialChar(trElement.getText()).trim().contains(strValue)){
            ...
            try {
                if ((replaceSpecialChar(AllContractPagesView.getTextAndByPassException(trElement)).trim().contains(strLabelName)) && replaceSpecialChar(AllContractPagesView.getTextAndByPassException(trElement)).trim().contains(strValue)) {
                    Log.info(" Value exist: " + strLabelName + " : " + strValue);
                    flag = true;
                    return flag;
                }
    ...
    public WebElement waitForElement(WebElement elementToWaitFor, Integer waitTimeInSeconds) {
        ...
        ele = wait.until(ExpectedConditions.elementToBeClickable((By) elementToWaitFor));
    ...
    public String addDaystoCurrentDateSkipWeekEndsCount(int Days, int daysCount) {
        SimpleDateFormat dateformat = new SimpleDateFormat("MM/dd/yyyy");
        Calendar cal = Calendar.getInstance();
//        int daysCount = 1;
        do {
    ...
    public void checkbox(WebElement ele, int timeout) throws Exception {
        waitForObjectToLoad(ele);
        Wait(timeout);
        if (ele.isDisplayed()) {
            if (!ele.isSelected())
                ele.click();
            Log.info("Selected checkbox ''" + ele.getText());
        } else {
            Exception error = new Exception(ele.getText());
    ...
    public void unCheckbox(String locator, String element, int timeout) throws Exception {
        WebElement ele = getWebElementObject(locator, element);
        waitForObjectToLoad(ele);
        Wait(timeout);
        if (ele.isDisplayed()) {
    ...
    public void unCheckbox(WebElement ele, int timeout) throws Exception {
        waitForObjectToLoad(ele);
        Wait(timeout);
        if (ele.isDisplayed()) {
            if (ele.isSelected())
    ...
    public boolean verifyUncheckbox(String locator, String element, int timeout) throws Exception {
    ...
    public boolean verifyUncheckbox(WebElement ele, int timeout) throws Exception {
        boolean boolFlag = true;
        waitForObjectToLoad(ele);
        Wait(timeout);
        if (ele.isDisplayed()) {
    ...
    public boolean verifycheckbox(String locator, String element, int timeout) throws Exception {
        boolean boolFlag = true;
        WebElement ele = getWebElementObject(locator, element);
        waitForObjectToLoad(ele);
        Wait(timeout);
    ...
    public boolean verifycheckbox(WebElement ele, int timeout) throws Exception {
        boolean boolFlag = true;
        waitForObjectToLoad(ele);
        Wait(timeout);
        if (ele.isDisplayed()) {
...
//            getClickableWebElementObject(locator, element).click();
    ...
    public void moveToElement(WebElement ele, int timeout) throws Exception {
        try {
            ...
            Log.info("unable move to element ''" + ele.getText());
        }
    ...
    public void escape(WebElement element, int timeout) throws Exception {
        waitForObjectToLoad(element);
        Wait(timeout);
        if (element.isDisplayed()) {
            Actions action = new Actions(driver);
            action.moveToElement(element).sendKeys(Keys.ESCAPE).build().perform();
            Log.info("escape element ''" + element.getText());
        } else {
            Exception error = new Exception();
            Log.error("Current page does not contain element  '" + element.getText());
            String Str = new String(error.getMessage());
            Log.error("Error Message :" + Str.substring(1, 240));
            throw error;
        }
    ...
    public String getFieldValue(String locator, String element, int timeout) throws Exception {
        String strGetValue = "";
        WebElement ele = getWebElementObject(locator, element);
        waitForObjectToLoad(ele);
        Wait(timeout);
        if (ele.isDisplayed()) {
            strGetValue = ele.getAttribute("value");
            Log.info("Typing text '" + ele.getAttribute("value") + "' into text field ''" + locator + "=" + element + "'");
            return strGetValue;
        } else {
            Exception error = new Exception(element);
            Log.error("Current page does not contain element ''" + locator + "=" + element + "'");
...
//    }
    ...
    public Boolean IsValuePresentInDropdown(String locator, String element, String strValue) throws Exception {
        WebElement eles = getWebElementObject(locator, element);
        waitForObjectToLoad(eles);
        for (WebElement ele : eles.findElements(By.tagName("option"))) {
    ...
    public Boolean IsValuePresentInDropdown(WebElement eles, String strValue) throws Exception {
        waitForObjectToLoad(eles);
        for (WebElement ele : eles.findElements(By.tagName("option"))) {
            if (ele.getText().equals(strValue)) {
                return true;
    ...
    public WebElement getWebElementObject(String locator, String element) throws Exception {
        ...
        try {
            ...
            WebElement webelement = wait.until(ExpectedConditions.presenceOfElementLocated(byElement));
    ...
    public WebElement getWebElementObjs(String locatorType, String locator, int counter, int timeOut) throws Exception {
        List<WebElement> eles = null;
        for (int i = 0; i < counter; i++) {
            try {
                BaseView.pushShortTimeout(1);
                ...
                for (WebElement ele : eles) {
                    WebDriverWait wait = new WebDriverWait(BrowserDriver.getCurrentDriver(), timeOut);
                    ele = wait.until(ExpectedConditions.visibilityOf(ele));
                    if (ele.isDisplayed()) return (WebElement) eles;
                }
            } catch (StaleElementReferenceException se) {
    ...
    public boolean verifyTextValue(String locator, String element, String strMsg, int timeout) {
        boolean flag = false;
        try {
            //moveToElement(locator, element, SeleniumUtilities.OBJWAITTIMEOUT);
            List<WebElement> eles = driver.findElements(getElementBy(locator, element));

            for (int i = 0; i < eles.size(); i++) {
                Actions action = new Actions(driver);
                //action.moveToElement(eles.get(i)).sendKeys("").build().perform();
...
//            ForceWaitForItem.delay(10000);
    ...
    public void dismissUnexpectedAlerts(String alertMessage) throws Exception {
        try {
            WebDriverWait wait = new WebDriverWait(BrowserDriver.getCurrentDriver(), 5);
            wait.until(ExpectedConditions.alertIsPresent());
            Alert alert = BrowserDriver.getCurrentDriver().switchTo().alert();
            if (alertMessage.trim().contains(alert.getText().trim())) {
                ...
                System.out.println("Alert not found and dismissed");
            }
            alert.dismiss();
        } catch (Exception e) {
        }
 ...
 */
    ...
    public WebElement getWebElementWithoutStaleness(String locator, String element) throws Exception {
        ...
        do {
            try {
                ...
                wait.until(ExpectedConditions.not(ExpectedConditions.stalenessOf(BrowserDriver.getCurrentDriver().findElement(byElement))));
    ...
    public static boolean waitForNonStaleWebElement(WebElement element, int counter, int timeOut) {
        ...
        for (int i = 0; i < counter; i++) {
            try {
                WebDriverWait wait = new WebDriverWait(BrowserDriver.getCurrentDriver(), timeOut);
                boolean ele = wait.until(ExpectedConditions.not(ExpectedConditions.stalenessOf(element)));
                if (ele) return ele;
            } catch (Exception e) {
                Log.info("Error: Waited for non stale element for " + timeOut + " seconds but no use...");
            } finally {
...
//            flag = true;
    ...
    public boolean waitForElementContainText(WebElement element, String text) throws Exception {
        ...
        try {
            ...
            textPresent = wait.until(ExpectedConditions.textToBePresentInElement(element, text));
        } catch (Exception ne) {
    ...
    public boolean waitForElementVisibilityOf(WebElement element, int timeOut) {
        try {
            BaseView.pushShortTimeout(1);
            WebDriverWait wait = new WebDriverWait(BrowserDriver.getCurrentDriver(), timeOut);
            return wait.until(ExpectedConditions.visibilityOf(element)).isDisplayed();
    ...
    public boolean waitForElementVisibilityOf(WebElement element, int counter, int timeOut) {
        ...
        for (int i = 0; i < counter; i++) {
            try {
                BaseView.pushShortTimeout(1);
                WebDriverWait wait = new WebDriverWait(BrowserDriver.getCurrentDriver(), timeOut);
                eleVisibile = wait.until(ExpectedConditions.visibilityOf(element)).isDisplayed();
    ...
    public boolean waitForElementIsClickable(String locatorType, String locator, int counter, int timeOut) {
        ...
        for (int i = 0; i < counter; i++) {
            try {
                ...
                WebElement element = getWebElementObject(locatorType, locator);
                WebDriverWait wait = new WebDriverWait(BrowserDriver.getCurrentDriver(), timeOut);
                eleVisibile = wait.until(ExpectedConditions.elementToBeClickable(element)).isDisplayed();
                if (eleVisibile) return eleVisibile;
            } catch (Exception ne) {
                Log.error("Webelement does not exist : " + locatorType + " : " + locator + "counter :" + i);
            } finally {
                BaseView.popDefaultTimeout();
            }
        }
...
//                wait.until(ExpectedConditions.not(ExpectedConditions.stalenessOf(element)));
    ...
    public boolean waitForElementNotPresent(String locatorType, String locator) throws Throwable {
        try {
            WebDriverWait wait = new WebDriverWait(BrowserDriver.getCurrentDriver(), WEBDRIVER_WAIT);
            wait.until(ExpectedConditions.invisibilityOfElementLocated(getElementBy(locatorType, locator)));
            return true;
        } catch (Exception e) {
            return false;
        }
    ...
    public boolean waitForElementPresent(String locatorType, String locator) throws Throwable {
        try {
            WebDriverWait wait = new WebDriverWait(BrowserDriver.getCurrentDriver(), WEBDRIVER_WAIT);
            wait.until(ExpectedConditions.visibilityOfElementLocated(getElementBy(locatorType, locator)));
            return true;
        } catch (Exception e) {
            return false;
        }
    ...
    public boolean waitUntilElementContainsText(String locatorType, String locator, String text, int timeOut) throws Exception {
        ...
        try {
            ...
            textPresent = wait.until(ExpectedConditions.textToBePresentInElement(getElement(locatorType, locator, timeOut), text));
    ...
    public boolean waitUntilElementNotContainsText(String locatorType, String locator, String text, int counter, int timeOut) throws Exception {
        ...
        for (int i = 0; i < counter; i++) {
            try {
                WebDriverWait wait = new WebDriverWait(BrowserDriver.getCurrentDriver(), timeOut);
                textPresent = wait.until(ExpectedConditions.not(ExpectedConditions.textToBePresentInElement(getElement(locatorType, locator, timeOut), text)));
                if (textPresent) return textPresent;
            } catch (Exception ne) {
                BaseView.takeScreenshot("TextNotFoundInElement.png");
                Log.info("Error :" + ne.getMessage());
    ...
    public boolean waitUntilElementNotContainsText(WebElement ele, String text, int counter, int timeOut) throws Exception {
        ...
        for (int i = 0; i < counter; i++) {
            try {
                WebDriverWait wait = new WebDriverWait(BrowserDriver.getCurrentDriver(), timeOut);
                textPresent = wait.until(ExpectedConditions.not(ExpectedConditions.textToBePresentInElement(ele, text)));
                if (textPresent) return textPresent;
            } catch (Exception ne) {
                BaseView.takeScreenshot("TextNotFoundInElement.png");
                Log.info("Error :" + ne.getMessage());
    ...
    public boolean waitUntilElementDisappers(String locatorType, String locator, int counter, int timeOut) throws Exception {
        ...
        for (int i = 0; i < counter; i++) {
            try {
                WebDriverWait wait = new WebDriverWait(BrowserDriver.getCurrentDriver(), timeOut);
                boolEleExist = wait.until(ExpectedConditions.not(ExpectedConditions.visibilityOf(getElement(locatorType, locator, timeOut))));
    ...
    public boolean waitUntilElementDisappers(WebElement ele, int counter, int timeOut) throws Exception {
        ...
        for (int i = 0; i < counter; i++) {
            try {
                WebDriverWait wait = new WebDriverWait(BrowserDriver.getCurrentDriver(), timeOut);
                boolEleExist = wait.until(ExpectedConditions.not(ExpectedConditions.visibilityOf(ele)));
                if (boolEleExist) return boolEleExist;
            } catch (Exception ne) {
                BaseView.takeScreenshot("waitUntilElementDisappers");
                Log.info("Error :" + ne.getMessage());
    ...
    public boolean textContains(String strSearchText) {
        ...
        try {
            ...
            return wait.until(ExpectedConditions.textToBePresentInElement(By.tagName("*"), strSearchText));
        } catch (Exception ex) {
    ...
    public String getCurrentDateWithHour() {
    ...
    public String getCurrentDateWithHourIn12HourFormat() {
        SimpleDateFormat dateformat = new SimpleDateFormat("MM/dd/yyyy hh a");
        Calendar cal = Calendar.getInstance();
        String newdate = dateformat.format(cal.getTime());
        Log.info("New date '" + newdate + "'");
        System.out.println(newdate);
    ...
    public String getCurrentDateWithHourIn12HourFormat(int Days) {
        SimpleDateFormat dateformat = new SimpleDateFormat("MM/dd/yyyy hh a");
        Calendar cal = Calendar.getInstance();
    ...
    public String getCurrentDateWithHourMinSecIn12HourFormat(int Days) {
    ...
    public String getCurrentDateMonthDayHour(int Days) {
        SimpleDateFormat dateformat = new SimpleDateFormat("EEE MMM dd yyyy HH:");
        Calendar cal = Calendar.getInstance();
        cal.add(Calendar.DATE, Days);
        String newdate = dateformat.format(cal.getTime());
        Log.info("New date '" + newdate + "'");
    ...
    public void pageRefresh() {
        ...
        System.out.println(BrowserDriver.getCurrentDriver().getTitle());

    }

    public void selectByIndex(String locator, String element, int index, int timeout) throws Exception {
        WebElement ele = getWebElementObject(locator, element);
        waitForObjectToLoad(ele);
        Wait(timeout);
        if (ele.isDisplayed()) {
            ...
            Log.info("Selected '" + index + "' from dropdown ''" + locator + "=" + element + "'");
        } else {
            Exception error = new Exception(element);
            Log.error("Current page does not contain element ''" + locator + "=" + element + "'");
            String Str = new String(error.getMessage());
