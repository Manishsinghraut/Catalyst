@all
Feature: Fixed Offset - Semi Annually-AnnualGrossUp Calculation (Ref : Jira Ticket No : Automation-673)
  ...
  Scenario Outline: I import file for Fixed Offset - Semi Annually-AnnualGrossUp Calculation
    ...
    And I import following file name:
      | 5FixedOffsetSemiAnnuallyAnnualGrossUp.xml |

  Examples:s
    | Login User   | Login full username  |
    | DeleteAccess | Delete Access Access |

  Scenario Outline: As a delete user I verify sales items of 5.Fixed Offset- Semi Annually- AnnualGrossUp entity
    Given I Login to IWMS using "<Login User>" access credentials
    Then I check previous user login "<Login full username>" and continue if user "<Login User>" same or logout and login if not
  ...
  Examples:
    ...
    | DeleteAccess | Delete Access Access | Contract | Payment Info     | 5.Fixed Offset- Semi Annually- AnnualGrossUp |


  Scenario Outline: As a delete user I verify Recurring expenses, vendor allocation, precent rent,offset and break point line items of 5.Fixed Offset- Semi Annually- AnnualGrossUp entity
    Given I Login to IWMS using "<Login User>" access credentials
    Then I check previous user login "<Login full username>" and continue if user "<Login User>" same or logout and login if not
    When I navigate to entity type "<Item>" named "<Entity Name>"
    Then I navigate to "<Second Level Tab>"
    ...
    And I verify a row from grid "BOGridPercentageRentBreakpoint" having unique value "01/01/2000" under column header "Begin Date"
      ...
      | Breakpoint Amount #2 | $100,000.00 |
  ...
  Scenario Outline: As a delete user I process payment of 5.Fixed Offset- Semi Annually- AnnualGrossUp entity
    Given I Login to IWMS using "<Login User>" access credentials
    Then I check previous user login "<Login full username>" and continue if user "<Login User>" same or logout and login if not
    When I navigate to entity type "<Item>" named "<Entity Name>"
    Then I navigate to "<Second Level Tab>"
...
#      | Sales      | June, 2014      | $200,000   | $0            | $1,250,000.00 | $25,000.00 | $25,000             | 10.00% | $62,500          |            | $62,500.00  |
      | Sales      | June, 2014      | $200,000   | $0            | $1,250,000    | $25,000    | $25,000             | 10.00% | $62,500.00       |            | $62,500.00  |
      ...
      | Sales      | December, 2014  | $200,000   | $0            | $2,380,000    | $50,000    | $50,000             | 10.00% | $119,000.00      |            | $119,000.00 |
#      | Sales      | December, 2014  | $200,000   | $0            | $2,380,000.00 | $50,000.00 | $50,000             | 10.00% | $119,000         |            | $119,000.00 |
      |            |                 | $2,380,000 | $0            |               |            |                     |        |                  | $0.00      |             |
    ...
    And I verify the "Percentage Rent (Single BP) Schedule" values at the bottom of the page
      ...
      | Offset Amount         | $1,200.00   |
      | Net Amount Due        | $117,800.00 |
    ...
    And I verify a row from grid "thisDiv" having unique value "$61,900.00" under column header "Total Amount"
      | name             | value             |
      | Group            | Lease Expense     |
      ...
      | Total Amount     | $61,900.00        |
      | Primary Tax      | $0.00             |
      | Hold Flag        | No                |

    And I verify a row from grid "thisDiv" having unique value "$55,900.00" under column header "Total Amount"
      | name             | value             |
      | Group            | Lease Expense     |
      | Type             | Percentage Rent   |
      | Category         | Expense           |
