@all
Feature: Fixed Offset Adjustment that does not span the whole PR year (Ref : Jira Ticket No : Automation-675)
  ...
  Scenario Outline: I import file for Fixed Offset - Semi Annually-AnnualGrossUp Calculation
    ...
    And I import following file name:
      | FixedOffsetNotSpanTheWholePRYear.xml |

    Examples:s
      | Login User   | Login full username  |
      | DeleteAccess | Delete Access Access |

  Scenario Outline: As a delete user I verify sales items of Fixed Offset not span the whole PR year entity
    Given I Login to IWMS using "<Login User>" access credentials
    Then I check previous user login "<Login full username>" and continue if user "<Login User>" same or logout and login if not
    ...
    Examples:
      ...
      | DeleteAccess | Delete Access Access | Contract | Payment Info     | Fixed Offset not span the whole PR year |


  Scenario Outline: As a delete user I verify Recurring expenses, vendor allocation, precent rent,offset and break point line items of Fixed Offset not span the whole PR year entity
    Given I Login to IWMS using "<Login User>" access credentials
    Then I check previous user login "<Login full username>" and continue if user "<Login User>" same or logout and login if not
    When I navigate to entity type "<Item>" named "<Entity Name>"
    Then I navigate to "<Second Level Tab>"
    ...
    And I verify a row from grid "BOGridVariableRentOffset" having unique value "06/01/2014" under column header "Begin Date"
      | name                | value              |
      | End Date            | 12/31/2014         |
      | Sales Group         | Sales              |
      | Expense Group       |                    |
  ...
  Scenario Outline: As a delete user I process payment of Fixed Offset not span the whole PR year entity
    Given I Login to IWMS using "<Login User>" access credentials
    Then I check previous user login "<Login full username>" and continue if user "<Login User>" same or logout and login if not
    When I navigate to entity type "<Item>" named "<Entity Name>"
    Then I navigate to "<Second Level Tab>"
    ...
    And I verify the "Percentage Rent (Single BP) Schedule" values at the bottom of the page
      | name                  | value       |
      | Total Percentage Rent | $233,000.00 |
      | Amount Already Paid   | $0.00       |
      | Percentage Rent Owed  | $233,000.00 |
      | Offset Amount         | $700.00     |
      | Net Amount Due        | $232,300.00 |
    And I select "June" month and "2014" year from dropdown in schedule details to view the details
    And I verify the "Percentage Rent (Single BP) Schedule" values at the bottom of the page
      | name                  | value       |
      | Total Percentage Rent | $122,500.00 |
      | Amount Already Paid   | $0.00       |
      | Percentage Rent Owed  | $122,500.00 |
      | Offset Amount         | $100.00     |
      | Net Amount Due        | $122,400.00 |
    And I select "January" month and "2015" year from dropdown in schedule details to view the details
    And I verify the "Percentage Rent (Single BP) Schedule" values at the bottom of the page
      | name                  | value |
      | Total Percentage Rent | $0.00 |
      | Amount Already Paid   | $0.00 |
      | Percentage Rent Owed  | $0.00 |
      | Offset Amount         | $0.00 |
      | Net Amount Due        | $0.00 |
    ...
    And I verify a row from grid "thisDiv" having unique value "$109,900.00" under column header "Total Amount"
      | name             | value             |
      | Group            | Lease Expense     |
      ...
      | Total Amount     | $109,900.00       |
      | Primary Tax      | $0.00             |
      | Hold Flag        | No                |
    And I verify a row from grid "thisDiv" having unique value "$122,400.00" under column header "Total Amount"
      | name             | value             |
      | Group            | Lease Expense     |
      | Type             | Percentage Rent   |
      | Category         | Expense           |
