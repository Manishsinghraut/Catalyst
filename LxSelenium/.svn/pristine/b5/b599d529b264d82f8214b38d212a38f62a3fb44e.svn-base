@all
Feature: Save Schedule Details Page to Document for Site (Ref : Jira Ticket No : AUTOMATION-1120)
  ...
  Scenario Outline: Save Site - Workflow to documents with different status
    Given I Login to IWMS using "<Login User>" access credentials
    Then I check previous user login "<Login full username>" and continue if user "<Login User>" same or logout and login if not
...
#      | DetailsSite |
  ...
  Examples:
    ...
    | DeleteAccess | Delete Access Access | DetailsPageSite | Details          | Schedule        | DetailsSite | Schedules     |